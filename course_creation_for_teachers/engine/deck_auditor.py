...
def audit_presentation(pptx_path):
    ...
    for idx, slide in enumerate(prs.slides):
        ...
        for shape in slide.shapes:
            ...
            # Check for Objectives, Battle Plan, Bibliography
            low_text = full_shape_text.lower()
            if "learning objectives" in low_text or ("objectives" in low_text and ("outcomes" in low_text or "competencies" in low_text)):
                has_objectives = True
            if "battle plan" in low_text or "alignment roadmap" in low_text:
                has_battle_plan = True
            if "references & further reading" in low_text or "bibliography" in low_text or "foundational literature" in low_text or "references" in low_text:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python deck_auditor.py <path_to_presentation.pptx>")
        sys.exit(1)
